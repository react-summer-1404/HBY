...
import data from "../NewsComponent/data.json";
              ...
              {isList === true
                ? courses?.courseFilterDtos?.map((item) => (
                    <div className="mx-auto mb-5 lg:mx-0" >
                      <NewsPageCard2 title={item.title} cost={item.cost} courseId={item.courseId} />
                    </div>
                  ))
                : courses?.courseFilterDtos?.map((item) => (
                    <div className="mx-auto mb-5 lg:mx-0" key={item.id}>
                      <NewsPageCard title={item.title} courseId={item.courseId}/>
                    </div>
                  ))}
            </div>
          </div>
